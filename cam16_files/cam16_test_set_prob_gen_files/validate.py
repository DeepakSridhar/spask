...
def main():
    ...
    log_file_name = 'mylog_'+str(dt.datetime.now().strftime('%d-%m-%Y %H:%M:%S'))+'.log'
    ...
    bce_loss,val_acc, out = calculate_epoch_loss(valloader_iter, valloader, model, 'val')
    with open('test_results_'+str(args.exp_name)+'.pkl','wb') as f:
        pickle.dump(out, f)
...
def calculate_epoch_loss(itera, loader, model, phase = 'train',cal = 'loss'):
            ...
            # outputs = torch.argmax(results, dim=1)
