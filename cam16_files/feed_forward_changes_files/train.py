...
def main():
    ...
    print(f"batch size: {args.batch_size}, train # of batches: {len(valloader)}")
    print(f"train # of samples: {len(val_dataset)}")
    print("##############################################")
    ...
    for k in tqdm(range(len(valloader))):
      try:
          _, batch_iter = itera.__next__()
      except:
          itera = enumerate(valloader)
          _, batch_iter = itera.__next__()
      ...
      print(result[2].cpu().numpy().dtype)

      x.extend(list(result[2].cpu().numpy()))
      y.extend(list(batch_iter['true_lab'].cpu().numpy()))

      gc.collect()

      # print(type(x),type(x[0]),x[0].shape)

    with open('test_set_5_slides_'+'.pkl','wb') as f:
      # pickle.dump((results.cpu().numpy(),targets.cpu().numpy()), f)
      pickle.dump((x,y), f)
    

    # for k in range(len(valloader)):
    #   try:
    #       _, batch_iter = itera.__next__()
    #   except:
    #       itera = enumerate(valloader)
    ...
    # for i_iter in range(args.num_steps):
        ...
        # # if i_iter % 10 == 0:
        # #     print(('iter = {:8d}/{:8d}, ' + 'train bce_loss = {:.3f}').format(i_iter, args.num_steps,train_bce_loss))
    ...
    # torch.save(model.state_dict(), osp.join(args.snapshot_dir,
    #                                         args.exp_name, 'model_' + str(i_iter)+"_"+str(dt.datetime.now().strftime('%d-%m-%Y %H:%M:%S')) + '.pth'))
