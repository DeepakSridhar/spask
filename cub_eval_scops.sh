...
nvidia-smi
read -e -p "Which GPU to use? : " -i "1" GPU
...
echo ""
read -e -p "Iter(s): "                     -i "20000" ITER


read -e -p "Save Dir: "                 -i "results_CUB_org" SAVE_DIR

read -e -p "extra args for test set : " -i "--crf" ARG_TEST
read -e -p "extra args for train set : " -i "--crf" ARG_TRAIN
...
do
    ...
    do
        ...
        # Testing
        CMD_TEST="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_cub_org.py $ARG_TEST --dataset $DATASET --data-dir $DATA_DIR --data-list $TEST_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/test/"
        ...
        then
            if [ ! -d "$SAVE_DIR/$method/ITER_$iter/train/" ]; then
                CMD_TRAIN="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_cub_org.py $ARG_TRAIN --dataset $DATASET --data-dir $DATA_DIR --data-list $TRAIN_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/train/"
