...
def img_resize(image, min_size=128):
    h, w, _ = image.shape
    # # if min(h, w) >= min_size:
    # #     return image

    # scalew = float(min_size) / w
    # scaleh = float(min_size) / h
    # new_scale = [int(scalew * w), int(scaleh * h)]

    new_image = Image.fromarray(image.astype(np.uint8)).resize([min_size, min_size], resample=Image.BILINEAR)
    new_image = np.asarray(new_image).astype(np.float32)
