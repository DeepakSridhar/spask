...
MODEL_DIR="snapshots_CelebA_Geometric_recon_8_sep"
...
ls $MODEL_DIR
echo ""
read -e -p "Model name(s): "             -i "SCOPS_K8_train_recon_8_sep"  METHOD

ls $MODEL_DIR/$METHOD/model*
echo ""
read -e -p "Iter(s): "                     -i "256000" ITER


read -e -p "Save Dir: "                 -i "results_CelebA_Geometric_sep" SAVE_DIR
...
do
    ...
    do
        ...
        # Testing
        CMD_TEST="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_celebAWild_mod.py $ARG_TEST --dataset $DATASET --data-dir $DATA_DIR --data-list $TEST_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/test/"
        ...
        then
            ...
            CMD_TRAIN="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_celebAWild_mod.py $ARG_TRAIN --dataset $DATASET --data-dir $DATA_DIR --data-list $TRAIN_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/train/"
