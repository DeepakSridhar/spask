...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        if args.save_viz:
            ...
            filename_gt = os.path.join(save_lm_dir, '{}_gt.png'.format(name[0][:-4]))
            Image.fromarray(imgs_viz[0,:,:,:].numpy().transpose(1, 2, 0).astype('uint8'), 'RGB').save(filename_gt)

            filename_pred = os.path.join(save_lm_dir, '{}_pred.png'.format(name[0][:-4]))
            ...
            # background mask index -1 
            back = output[:,0,:,:].unsqueeze(1)
            output[:,1:,:,:] *= (1 - back) * label.unsqueeze(1).cuda()
