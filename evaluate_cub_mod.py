...
def get_arguments():
    ...
    parser.add_argument("--lm-count", type=int, default=15,
...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        image = batch['img']
        label = batch['mask']
        landmarks_gt[index,:,:] = batch['kp'][:,:,:2]#[0, random.sample([i for i in range(15)], args.num_parts),:2]
        img_path = batch['img_path']
        tmp = img_path[0].split("/")
        img_folder = tmp[-2]
        img_nm = tmp[-1].replace(".jpg", ".png")
        # print(img_nm, img_folder, tmp)
        ...
        input_tensor_cam = ((image.numpy() + IMG_MEAN.reshape((1, 3, 1, 1))) / 255.0).clip(0, 1.0)
        ...
        if args.save_viz:
            imgs_viz = image * 255.0
            future_im_pred = out_dict['future_im_pred'].detach().cpu()
            
            pred_imgs_viz = future_im_pred.numpy()*255.0

            # filename_gt = os.path.join(save_lm_dir, '{}_gt.png'.format(img_nm))
            # Image.fromarray(imgs_viz[0,:,:,:].cpu().numpy().transpose(1, 2, 0).astype('uint8'), 'RGB').save(filename_gt)
            
            filename_pred = os.path.join(save_lm_dir, '{}_pred.png'.format(img_nm))
            ...
            # For 3 parts CUB mask background index -1
            back = output[:,0,:,:].unsqueeze(1)
            output[:,1:,:,:] *= (1 - back) 
            output[:,1:,:,:] *= (1 - output[:,-1,:,:].unsqueeze(1))               
            output[:,-1,:,:] = 0.1
            output[:,0,:,:] = 0.1
