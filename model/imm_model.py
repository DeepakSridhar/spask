...
class PoseEncoder(nn.Module):
    ...
    def __init__(self, in_channels, n_filters, n_maps, map_sizes, gauss_std=0.1, gauss_mode='ankush'):
        ...
        # self.conv = MobileNet_GDConv(n_maps)
    ...
    def __init__(self, map_size, map_filters, n_filters, n_final_out, n_final_res, batch_norm=True):
        ...
        while map_size[0] <= n_final_res:
            ...
            if map_size[0] == n_final_res:
                ...
                # self.seq_renderers.add_module('sigmoid', nn.Sigmoid())
                break
            else:
...
class AssembleNetV1(nn.Module):
    ...
    def __init__(self, in_channels=3, n_filters=32, n_maps=10, gauss_std=0.1, \
            ...
            max_size=[256, 256], min_size=[32, 32]):
    ...
    def forward(self, embeddings, future_im, im=None):
        if im is not None:
        ...
        if im is not None:
            gauss_pt, pose_embeddings = self.pose_encoder(im)
...
class AssembleNet(nn.Module):
    ...
    def forward(self, embeddings, pose_embeddings):
        
        joint_embedding = torch.cat((embeddings, pose_embeddings), dim=1)
        future_im_pred = self.renderer(joint_embedding)

        return future_im_pred
