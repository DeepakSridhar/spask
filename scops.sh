...
do
    ...
    do
        ...
        # Testing
        CMD_TEST="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_celebAWild_org.py $ARG_TEST --dataset $DATASET --data-dir $DATA_DIR --data-list $TEST_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/test/"
        ...
        then
            if [ ! -d "$SAVE_DIR/$method/ITER_$iter/train/" ]; then
                CMD_TRAIN="CUDA_VISIBLE_DEVICES=${GPU} python evaluate_celebAWild_org.py $ARG_TRAIN --dataset $DATASET --data-dir $DATA_DIR --data-list $TRAIN_LIST --num-parts $NUM_PARTS --model $MODEL --restore-from $SNAPSHOT --save-dir $SAVE_DIR/$method/ITER_$iter/train/"
