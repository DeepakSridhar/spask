...
def generate_transformer_matrices(batch_size=32, img_sz=128, translate=0.1, random_scale=(0.7,1.1), rotate=60):
    ...
    return theta, angle_random


class IMGAUG(object):
    def __init__(self, min_scale=0.01, max_scale=0.1):
        import imgaug.augmenters as iaa
        self.seq = iaa.Sequential([
            iaa.PiecewiseAffine(scale=(min_scale, max_scale)),
        ])
    
    def forward(self, x):
        images_aug = self.seq(images=x)
        return images_aug
...
class RandTPS(nn.Module):
    ...
    def reset_control_points(self):
        ...
        if self.mode == 'affine':
            ...
            theta, angle = generate_transformer_matrices(batch_size=self.batch_size, img_sz=2.0, random_scale=self.random_scale)
