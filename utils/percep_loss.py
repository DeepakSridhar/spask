...
class PercepLoss(nn.Module):
    ...
    """
    def __init__(self, args, loss_type='perceptual'):
        super(PercepLoss, self).__init__()
        self.loss_type = loss_type
        self.ema = EMA()
        self.vggnet = Vgg16() if loss_type == 'perceptual' else None
        # self.vggnet = FeatureExtraction(
            # feature_extraction_cnn=args.ref_net, normalization=args.ref_norm, last_layer='conv5_2', output_tuple=True)
        self.vggnet.eval()
        self._init_ema()
