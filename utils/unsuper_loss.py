...
def PointLoss(angle, a_pos, a_scr, pos_A, pos_B, scr_A, scr_B):
    ...
    RepeatPoint = torch.mean(scr_mean * (Distance - PositionPoint))
...
def PointPredLoss(a_xy, pos_r):
    ...
    loss_x = torch.pow((pos_X - torch.div((pos_X_indices.float() - 1),(H * W - 1))),2)
    loss_y = torch.pow((pos_Y - torch.div((pos_Y_indices.float() - 1),(H * W - 1))),2)
...
def DescLoss(lambda_d, pos_A, pos_B, angle, desc_A, desc_B, margin_p, margin_n):
    ...
    pos_A = pos_A.view(2, H * W).transpose(0,1)#.transpose(1,2)
    pos_B = pos_B.view(2, H * W).transpose(0,1)#.transpose(1,2)
    # print(pos_A.shape)
    pos_dist = torch.cdist(pos_A,pos_B).unsqueeze(0)
    ...
    loss = torch.mean(lambda_d * ConstantMatrix * product_p + (1 - ConstantMatrix) * product_n)
...
def ComputeLoss(angle, scr_A, scr_B, pos_A_r, pos_B_r, pos_A, pos_B, desc_A, desc_B):
    ...
    # print(p_loss, pred_loss, d_loss, deco_loss)
