...
def Batch_Draw_Unsup_Landmarks(imgs, lms):
...
def Batch_Get_Centers(pred, sm=True):
    ...
    for b in range(B):
        for c in range(1,C):
            ...
            y_c = (y_c+1.0)/2*H
            centers[b,c-1,:] = torch.tensor([x_c,y_c])
    return centers

def Batch_Pred_Landmarks(pred, sm=True):
    ...
    centers = torch.zeros((B,C-1,2)).cuda()
...
def Batch_Draw_Landmarks(imgs, pred, sm=True):
    ...
    for b in range(B):
        for c in range(1,C):
            ...
            centers[b,c-1,:] = [x_c.cpu()/2,y_c.cpu()/2]
            ...
            img = imgs_cv2[b].copy()
            cv2.drawMarker(img, (int(x_c),int(y_c)), (int(cmap[c][0]), int(cmap[c][1]), int(cmap[c][2])), markerType=cv2.MARKER_CROSS, markerSize = 10, thickness=2)
            imgs_cv2[b] = img
...
class Visualizer(object):
    ...
    def vis_image_only(self, i_iter, imgs, mean, name='Predicted transform'):
        if i_iter % self.vis_interval == 0 :
            ...
            imgs_viz_grid = vutils.make_grid(imgs, normalize=False, scale_each=False)
            self.tb_writer.add_image(f'{name}', imgs_viz_grid, i_iter)
    ...
    def vis_unsup_landmarks(self, i_iter, imgs, mean, lms, name='Pred'):
        if i_iter % self.vis_interval == 0 :
            ...
            lm_viz = Batch_Draw_Unsup_Landmarks(imgs_viz, lms)
            lm_viz = torch.tensor(lm_viz.astype(np.float32))

            lm_viz = vutils.make_grid(lm_viz/255.0, normalize=False, scale_each=False)
            self.tb_writer.add_image('Landmark_{}'.format(name), lm_viz, i_iter)
